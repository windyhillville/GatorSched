...
def decline_swap(request_id: int, db: Session) -> RequestDecisionResponse:
    swap_request_stmt = select(SwapRequest).where(SwapRequest.id == request_id)
    swap_request = db.scalars(swap_request_stmt).unique().one()

    swap_request.employee_status = EmployeeRequestStatus.rejected
    swap_request.manager_status = ManagerRequestStatus.not_sent
